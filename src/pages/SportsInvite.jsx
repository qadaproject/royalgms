...
import { Bell, Send, Mail, MessageSquare, CheckCircle2, Loader2, Search, Phone, AlertTriangle, Globe } from "lucide-react";
...
const DEFAULT_EMAIL_TEMPLATE = `{{name}}`;
const DEFAULT_SMS_TEMPLATE = `Hello {{name}}, you are cordially invited to the Warri Games. Download your invitation: {{link}}`;
const DEFAULT_EMAIL_SUBJECT = `Warri Games — Royal Invitation`;

export default function SportsInvite() {
  ...
  const sendNotification = async (guest, ch) => {
    ...
    base44.entities.GuestActivityLog.create({
      ...
      description: `${ch} Warri Games invitation ${success ? "sent" : "failed"}`,
  ...
  const sendBulk = async () => {
    ...
    toast.success(`Bulk invitations sent to ${unsent.length} guests`);
  };
  ...
  return (
    <div>
      <PageHeader title="Sports Invite" subtitle="Send Warri Games invitations to guests via WhatsApp, Email & SMS">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
          ...
          {filteredLogs.length === 0 ? (
            <p className="text-muted-foreground text-sm">{logs.length === 0 ? "No invitations sent yet" : "No logs match filters"}</p>
          ) : (
          ...
          {confirmGuest && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                You are about to send a <strong>{confirmGuest.channel}</strong> Warri Games invitation to:
              </p>
